...
function parseCommandStrings(input) {
    let arr = null;
    ...
    if (!arr || arr.length === 0)
        return DEFAULT_FRAGMENT_STRINGS;
    return arr;
}
...
function buildFragmentTexture(str, colorHex) {
...
export default function TerminalStage(props) {
    const { rootRef, size, input, reducedMotion, screenshot, screenshotX, screenshotY, screenshotScale, screenshotTilt, screenEmissive, fragmentCount, commands, fragmentColor1, fragmentColor2, fragmentColor3, fragmentSpeed, particleCount, particleColor1, particleColor2, particleColor3, particlePull, cursorTrailEnabled, cursorTrailColor, cursorTrailLength, haloColor, haloStrength, parallaxStrength, } = props;
    ...
    const commandStringsRef = React.useRef(commandStrings);
    ...
    React.useEffect(() => {
        ...
        for (let i = 0; i < MAX_FRAGMENTS; i++) {
            ...
            if (Math.abs(bx) < 1.6)
                bx += Math.sign(bx || 1) * (1.0 + r4 * 0.8);
            if (Math.abs(by) < 1.0)
                by += Math.sign(by || 1) * (0.6 + r5 * 0.5);
        ...
        return () => {
            ...
            scene.traverse((obj) => {
                ...
                if (obj.material) {
                    ...
                    for (const m of mats) {
                        if (m.map && m.map !== state.screenTex)
                            m.map.dispose?.();
                        m.dispose?.();
                    }
                }
    ...
    React.useEffect(() => {
        ...
        if (!state.scene || !state.screenMat)
            return;
        if (!screenshot) {
            if (state.screenMat) {
        ...
        const applyTexture = (tex, img) => {
            ...
            if (img && img.width && img.height) {
                state.screenAspect = img.width / img.height;
            }
            if (state.screenMat) {
        ...
        if (preImg) {
            ...
            const onError = (err) => { console.error('[cyber-terminal] screenshot load failed:', err); setAssetReady(true); };
            ...
            loader.load(screenshot, (tex) => applyTexture(tex, tex.image), undefined, (err) => {
    ...
    React.useEffect(() => {
        ...
        if (!list.length || !state.fragments.length)
            return;
        const colorKeys = [fragmentColor1, fragmentColor2, fragmentColor3];
        const scaleK = 0.0042;
    ...
    React.useEffect(() => {
        ...
        if (state.pGeom)
            state.pGeom.setDrawRange(0, partN);
        // Trail color.
        ...
        if (state.pCol && state.pGeom) {
            ...
            state.pGeom.attributes.color.needsUpdate = true;
        }
        ...
        for (let i = 0; i < state.fragments.length; i++) {
            ...
            f.mat.color.setRGB(0.85 + 0.15 * col.r, 0.85 + 0.15 * col.g, 0.85 + 0.15 * col.b);
        }
        // Trail line visibility (we still rebuild trail every frame so the
        // `visible` flag is the only sync to do here).
        if (state.trailLine)
            state.trailLine.visible = !!cursorTrailEnabled;
    }, [
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "crazygl-cyber-terminal-canvas", "aria-hidden": "true" }));
}
